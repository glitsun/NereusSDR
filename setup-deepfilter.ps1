...
.DESCRIPTION
    Downloads a pre-built libdeepfilter DLL from the NereusSDR release tagged
    'dfnr-libs', or falls back to cloning the DeepFilterNet repository and
    building the C API library (DLL) using Cargo with the MSVC target. Copies
    the output to third_party/deepfilter/ ready for CMake. Also generates a
    MinGW import library if dlltool is available.

    Requires: Rust toolchain (cargo), git, Visual Studio 2022 (for MSVC target).
    Override the release repository with $env:DFNR_RELEASE_REPO.
...
# Override the repo with $env:DFNR_RELEASE_REPO if building from a fork.
$ReleaseRepo = if ($env:DFNR_RELEASE_REPO) { $env:DFNR_RELEASE_REPO } else { "boydsoftprez/NereusSDR" }
